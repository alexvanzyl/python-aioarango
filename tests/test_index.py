...
async def test_list_indexes(icol: StandardCollection, bad_col):
...
async def test_add_hash_index(icol: StandardCollection):
    icol = icol
    ...
    assert result["id"] in extract("id", await icol.indexes())

    # Clean up the index
    await icol.delete_index(result["id"])


async def test_add_skiplist_index(icol: StandardCollection):
    fields = ["attr1", "attr2"]
...
async def test_add_geo_index(icol: StandardCollection):
...
async def test_add_fulltext_index(icol: StandardCollection):
...
async def test_add_persistent_index(icol: StandardCollection):
...
async def test_add_ttl_index(icol: StandardCollection):
    # Test add persistent index with two attributes
...
async def test_delete_index(icol: StandardCollection, bad_col: StandardCollection):
    old_indexes = set(extract("id", await icol.indexes()))
    await icol.add_hash_index(["attr3", "attr4"], unique=True)
    await icol.add_skiplist_index(["attr3", "attr4"], unique=True)
    await icol.add_fulltext_index(fields=["attr3"], min_length=10)

    new_indexes = set(extract("id", await icol.indexes()))
...
async def test_load_indexes(icol: StandardCollection, bad_col: StandardCollection):
    # Test load indexes
