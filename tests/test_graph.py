...
from python_aioarango.collection import EdgeCollection, VertexCollection
...
async def test_graph_properties(graph: Graph, bad_graph: Graph, db: StandardDatabase):
...
async def test_graph_management(db: StandardDatabase, bad_db: StandardDatabase):
    # Test create graph
    ...
    assert graph_name in extract("name", await db.graphs())
    ...
    with assert_raises(GraphListError) as err:
        await bad_db.has_graph(graph_name)
    assert err.value.error_code in {11, 1228}

    # Test delete graph
    assert await db.delete_graph(graph_name) is True
    assert graph_name not in extract("name", await db.graphs())
    ...
    collections = extract("name", await db.collections())
...
async def test_vertex_collection_management(
    db: StandardDatabase, graph: Graph, bad_graph: Graph
):
    ...
    assert fvcol_name in extract("name", await db.collections())

    # Test has vertex collection with bad graph
    with assert_raises(VertexCollectionListError) as err:
        await bad_graph.has_vertex_collection(fvcol_name)
    assert err.value.error_code in {11, 1228}
    ...
    assert tvcol_name in extract("name", await db.collections())
    ...
    assert fvcol_name in extract("name", await db.collections())
    assert tvcol_name not in extract("name", await db.collections())
    assert not await graph.has_vertex_collection(tvcol_name)
...
async def test_edge_definition_management(
    db: StandardDatabase, graph: Graph, bad_graph: Graph
):
    ecol_name = generate_col_name()
    ...
    assert ecol_name in extract("name", await db.collections())
    ...
    with assert_raises(EdgeDefinitionListError) as err:
        await bad_graph.has_edge_definition(ecol_name)
    assert err.value.error_code in {11, 1228}
    ...
    assert bad_vcol_name in extract("name", await db.collections())
    assert bad_vcol_name in extract("name", await db.collections())
    ...
    assert ecol_name not in extract("edge_collection", await graph.edge_definitions())
    assert not await graph.has_edge_definition(ecol_name)
    assert not await graph.has_edge_collection(ecol_name)
    assert ecol_name not in extract("name", await db.collections())


async def test_create_graph_with_edge_definition(db: StandardDatabase):
    new_graph_name = generate_graph_name()
...
async def test_vertex_management(
    fvcol: VertexCollection, bad_fvcol: VertexCollection, fvdocs
):
    # Test insert vertex with no key
    result = await fvcol.insert({})
    assert await fvcol.has(result["_key"], check_rev=False)
    ...
    assert await fvcol.has("foo", check_rev=False)
    assert await fvcol.has(vertex_id, check_rev=False)
    assert await fvcol.count() == 1
    await empty_collection(fvcol)
    ...
    assert await fvcol.has(vertex, check_rev=False) and fvcol.has(key, check_rev=False)
    assert await fvcol.count() == 1
    ...
    assert result["_key"] == key
    assert "_rev" in result
    assert await fvcol.has(vertex, check_rev=False) and await fvcol.has(
        key, check_rev=False
    )
    ...
    result = await fvcol.update(
        {"_key": key, "foo": 100}, return_old=True, return_new=True
    )
    ...
    result = await fvcol.replace(
        {"_key": key, "baz": 100}, return_old=True, return_new=True
    )
    assert "old" in result
    assert "new" in result
    assert "vertex" in result
    ...
    assert result["_key"] == key
    assert result["_old_rev"] == old_rev
    assert clean_doc((await fvcol.get(key))) == vertex
    old_rev = result["_rev"]
    ...
    assert result["_key"] == key
    assert result["_old_rev"] == old_rev
    assert clean_doc((await fvcol.get(key))) == clean_doc(vertex)
    old_rev = result["_rev"]
    ...
    if fvcol.context != "transaction":
        ...
        assert err.value.error_code in {1200, 1903}
        vertex["_rev"] = old_rev
        assert await fvcol.has(vertex, check_rev=False)
    ...
    assert not (await fvcol.has(vertex, check_rev=False))
    assert await fvcol.count() == 2
    ...
    await fvcol.import_bulk(fvdocs)
    assert await fvcol.count() == len(fvdocs)
    await empty_collection(fvcol)

    await fvcol.insert_many(fvdocs)
    assert await fvcol.count() == len(fvdocs)

    await fvcol.delete_many(fvdocs)
    assert await fvcol.count() == 0


async def test_vertex_management_via_graph(graph: Graph, fvcol: VertexCollection):
    ...
    assert not (await fvcol.has(vertex_id))
    assert await fvcol.count() == 0


async def test_edge_management(
    ecol: EdgeCollection,
    bad_ecol: EdgeCollection,
    edocs,
    fvcol: VertexCollection,
    fvdocs,
    tvcol: VertexCollection,
    tvdocs,
):
    ...
    assert await ecol.has(result["_key"], check_rev=False)
    ...
    assert "new" in result
    assert await ecol.has(result["edge"]["_key"], check_rev=False)
    assert await ecol.count() == 1
    await empty_collection(ecol)

    # Test insert vertex with ID
    edge_id = ecol.name + "/" + "foo"
    await ecol.insert({"_id": edge_id, "_from": edge["_from"], "_to": edge["_to"]})
    ...
    assert result["_key"] == key
    assert "_rev" in result
    assert await ecol.has(edge, check_rev=False) and await ecol.has(
        key, check_rev=False
    )
    assert await ecol.count() == 1
    ...
    if ecol.context != "transaction":
        ...
        assert err.value.error_code in {1200, 1903}
        edge["_rev"] = old_rev
        assert await ecol.has(edge, check_rev=False)
    ...
    assert not (await ecol.has(edge, check_rev=False))
    ...
    await ecol.import_bulk(edocs)
    assert await ecol.count() == len(edocs)
    await empty_collection(ecol)

    await ecol.insert_many(edocs)
    assert await ecol.count() == len(edocs)

    await ecol.delete_many(edocs)
    assert await ecol.count() == 1


async def test_vertex_edges(db: StandardDatabase, bad_db: StandardDatabase):
    graph_name = generate_graph_name()
...
async def test_edge_management_via_graph(
    graph: Graph,
    ecol: EdgeCollection,
    fvcol: VertexCollection,
    fvdocs,
    ...
    assert not (await ecol.has(edge_id, check_rev=False))
    assert await ecol.count() == 1


async def test_traverse(db: StandardDatabase):
    # Create test graph, vertex and edge collections
    ...
    await teaches.insert(
        {"_from": f"{profs.name}/anna", "_to": f"{classes.name}/CSC101"}
    )
    await teaches.insert(
        {"_from": f"{profs.name}/anna", "_to": f"{classes.name}/STA201"}
    )
    await teaches.insert(
        {"_from": f"{profs.name}/anna", "_to": f"{classes.name}/MAT223"}
    )
    await teaches.insert(
        {"_from": f"{profs.name}/andy", "_to": f"{classes.name}/MAT101"}
    )
    await teaches.insert(
        {"_from": f"{profs.name}/andy", "_to": f"{classes.name}/MAT102"}
    )
    await teaches.insert(
        {"_from": f"{profs.name}/andy", "_to": f"{classes.name}/MAT223"}
    )

    # Traverse the graph with default settings
    result = await school.traverse(f"{profs.name}/anna")
    ...
    result = await school.traverse(f"{profs.name}/andy")
    visited = extract("_key", result["vertices"])
    ...
    with assert_raises(GraphTraverseError):
        ...
        await school.traverse(f"{bad_col_name}/hanna")

    with assert_raises(GraphTraverseError):
        await school.traverse(f"{profs.name}/anderson")

    # Travers the graph with max iteration of 0
    with assert_raises(GraphTraverseError):
        await school.traverse(f"{profs.name}/andy", max_iter=0)

    # Traverse the graph with max depth of 0
    result = await school.traverse(f"{profs.name}/andy", max_depth=0)
    assert extract("_key", result["vertices"]) == ["andy"]

    result = await school.traverse(f"{profs.name}/anna", max_depth=0)
    assert extract("_key", result["vertices"]) == ["anna"]

    # Traverse the graph with min depth of 2
    result = await school.traverse(f"{profs.name}/andy", min_depth=2)
    assert extract("_key", result["vertices"]) == []

    result = await school.traverse(f"{profs.name}/anna", min_depth=2)
    assert extract("_key", result["vertices"]) == []
