...
from python_aioarango.collection import StandardCollection
from python_aioarango.database import StandardDatabase
from python_aioarango.exceptions import (
    CursorCloseError,
    CursorNextError,
    DocumentCountError,
    DocumentDeleteError,
...
async def test_document_insert(col: StandardCollection, docs):
    # Test insert document with no key
    result = await col.insert({})
    assert await col.has(result["_key"], check_rev=False)
    ...
    assert await col.has("foo", check_rev=False)
    assert await col.has(doc_id, check_rev=False)
    assert await col.count() == 1
    await empty_collection(col)
    ...
    result = await col.insert(
        document=doc, overwrite=True, return_old=True, return_new=True
    )
...
async def test_document_insert_many(
    col: StandardCollection, bad_col: StandardCollection, docs
):
...
async def test_document_update(col: StandardCollection, docs):
    doc = docs[0]
    await col.insert(doc)
...
async def test_document_update_many(
    col: StandardCollection, bad_col: StandardCollection, docs
):
...
async def test_document_update_match(
    col: StandardCollection, bad_col: StandardCollection, docs
):
    ...
    assert (
        await col.update_match({"val": 3}, {"val": None}, sync=True, keep_none=True)
        == 1
    )
    ...
    assert (
        await col.update_match({"val": 1}, {"val": None}, sync=False, keep_none=False)
        == 2
    )
...
async def test_document_replace(col: StandardCollection, docs):
    doc = docs[0]
    await col.insert(doc)
...
async def test_document_replace_many(
    col: StandardCollection, bad_col: StandardCollection, docs
):
    await col.insert_many(docs)
...
async def test_document_replace_match(
    col: StandardCollection, bad_col: StandardCollection, docs
):
    await col.import_bulk(docs)
...
async def test_document_delete(col: StandardCollection, docs):
    # Set up test documents
    await col.import_bulk(docs)
    ...
    assert isinstance(result["_rev"], str)
    assert "old" not in result
    assert not (await col.has(doc["_key"], check_rev=False))
    ...
    assert await col.has(bad_doc["_key"], check_rev=False)
    assert await col.count() == 3
...
async def test_document_delete_many(
    ...
    for result, doc_key in zip(results, doc_keys):
        ...
        assert isinstance(result["_rev"], str)
        assert "old" not in result
        assert not (await col.has(doc_key, check_rev=False))
        old_revs[doc_key] = result["_rev"]
...
async def test_document_delete_match(
    col: StandardCollection, bad_col: StandardCollection, docs
):
    # Set up test documents
    await col.import_bulk(docs)
...
async def test_document_find(
    ...
    found = list([doc async for doc in await col.find({"val": 2})])
    assert len(found) == 1
    assert found[0]["_key"] == "2"

    # Test find (multiple matches) with default options
    await col.update_match({"val": 2}, {"val": 1})
    found = list([doc async for doc in await col.find({"val": 1})])
    assert len(found) == 2
    for doc in map(dict, found):
        assert doc["_key"] in {"1", "2"}
        assert await col.has(doc["_key"], check_rev=False)
    ...
    found = list([doc async for doc in await col.find({"val": 1}, skip=100)])
    assert len(found) == 0

    found = list([doc async for doc in await col.find({"val": 1}, skip=0)])
    assert len(found) == 2

    found = list([doc async for doc in await col.find({"val": 1}, skip=1)])
    assert len(found) == 1
    for doc in map(dict, found):
    ...
    for limit in [3, 4, 5]:
        found = list([doc async for doc in await col.find({}, limit=limit)])
    ...
    await empty_collection(col)
    assert list([doc async for doc in await col.find({})]) == []
    assert list([doc async for doc in await col.find({"val": 1})]) == []
    assert list([doc async for doc in await col.find({"val": 2})]) == []
    assert list([doc async for doc in await col.find({"val": 3})]) == []
    assert list([doc async for doc in await col.find({"val": 4})]) == []
...
async def test_document_find_near(
    col: StandardCollection, bad_col: StandardCollection, docs
):
    await col.import_bulk(docs)

    # Test find_near with default options
    result = [doc async for doc in await col.find_near(latitude=1, longitude=1)]
    ...
    result = [
        doc async for doc in await col.find_near(latitude=1, longitude=1, limit=0)
    ]
    assert extract("_key", result) == []

    # Test find_near with limit of 1
    result = [
        doc async for doc in await col.find_near(latitude=1, longitude=1, limit=1)
    ]
    assert extract("_key", result) == ["1"]

    # Test find_near with limit of 3
    result = [
        doc async for doc in await col.find_near(latitude=1, longitude=1, limit=3)
    ]
    assert extract("_key", result) == ["1", "2", "3"]

    # Test find_near with limit of 3 (another set of coordinates)
    result = [
        doc async for doc in await col.find_near(latitude=5, longitude=5, limit=3)
    ]
    ...
    assert list(result) == []
    result = [
        doc async for doc in await col.find_near(latitude=5, longitude=5, limit=4)
    ]
    assert list(result) == []
...
async def test_document_find_in_range(
    col: StandardCollection, bad_col: StandardCollection, docs
):
    await col.import_bulk(docs)

    # Test find_in_range with default options
    result = [doc async for doc in await col.find_in_range("val", lower=1, upper=2)]
    assert extract("_key", result) == ["1"]
    ...
    result = [
        doc async for doc in await col.find_in_range("val", lower=1, upper=2, limit=0)
    ]
    assert extract("_key", result) == []

    # Test find_in_range with limit of 3
    result = [
        doc async for doc in await col.find_in_range("val", lower=1, upper=5, limit=3)
    ]
    assert extract("_key", result) == ["1", "2", "3"]
    ...
    result = [
        doc async for doc in await col.find_in_range("val", lower=1, upper=5, skip=0)
    ]
    assert extract("_key", result) == ["1", "2", "3", "4"]

    # Test find_in_range with skip set to 3
    result = [
        doc async for doc in await col.find_in_range("val", lower=1, upper=5, skip=2)
    ]
...
async def test_document_find_in_radius(
    col: StandardCollection, bad_col: StandardCollection
):
    doc1 = {"_key": "1", "loc": [1, 1]}
    doc2 = {"_key": "2", "loc": [1, 4]}
    doc3 = {"_key": "3", "loc": [4, 1]}
    ...
    result = list(
        [
            doc
            async for doc in await col.find_in_radius(
                latitude=1,
                longitude=4,
                radius=6,
            )
    ...
    result = list(
        [
            doc
            async for doc in await col.find_in_radius(
                latitude=1, longitude=1, radius=6, distance_field="dist"
            )
        ]
    )
    assert len(result) == 1
...
async def test_document_find_in_box(
    col: StandardCollection, bad_col: StandardCollection, geo: dict, cluster
):
    if cluster:
    ...
    result = [
        doc
        async for doc in await col.find_in_box(
...
async def test_document_find_by_text(col: StandardCollection, docs):
    await col.import_bulk(docs)

    # Test find_by_text with default options
    result = [
        doc async for doc in await col.find_by_text(field="text", query="foo,|bar")
    ]
    ...
    result = [
        doc async for doc in await col.find_by_text(field="text", query="foo", limit=0)
    ]
    assert len(list(result)) == 0

    result = [
        doc async for doc in await col.find_by_text(field="text", query="foo", limit=1)
    ]
    assert len(list(result)) == 1

    result = [
        doc async for doc in await col.find_by_text(field="text", query="foo", limit=2)
    ]
    assert len(list(result)) == 2

    result = [
        doc async for doc in await col.find_by_text(field="text", query="foo", limit=3)
    ]
...
async def test_document_has(col: StandardCollection, bad_col: StandardCollection, docs):
...
async def test_document_get(col: StandardCollection, bad_col: StandardCollection, docs):
    # Set up test documents
    await col.import_bulk(docs)
    doc = docs[0]
    doc_val = doc["val"]
...
async def test_document_get_many(
    col: StandardCollection, bad_col: StandardCollection, docs
):
    # Set up test documents
    await col.import_bulk(docs)
...
async def test_document_all(col: StandardCollection, bad_col: StandardCollection, docs):
    # Set up test documents
    await col.import_bulk(docs)

    # Test all with default options
    cursor = await col.all()
    result = list([doc async for doc in cursor])
    assert clean_doc(result) == docs
...
async def test_document_ids(col: StandardCollection, bad_col: StandardCollection, docs):
...
async def test_document_export(
    col: StandardCollection, bad_col: StandardCollection, docs, cluster
):
    if cluster:
        pytest.skip("Not tested in a cluster setup")
    ...
    assert clean_doc([doc async for doc in cursor]) == docs
    ...
    assert clean_doc(await cursor.next()) in docs
...
async def test_document_random(
    col: StandardCollection, bad_col: StandardCollection, docs
):
    # Set up test documents
    await col.import_bulk(docs)
...
async def test_document_import_bulk(
    col: StandardCollection, bad_col: StandardCollection, docs
):
    ...
    for doc in docs:
        doc_key = doc["_key"]
        assert await col.has(doc_key, check_rev=False)
    ...
    result = await col.import_bulk([new_doc], on_duplicate="error", halt_on_error=False)
    assert await col.count() == 1
    assert result["created"] == 0
    ...
    result = await col.import_bulk(
        [new_doc], on_duplicate="ignore", halt_on_error=False
    )
    assert await col.count() == 1
    assert result["created"] == 0
    assert result["errors"] == 0
    ...
    result = await col.import_bulk(
        [new_doc], on_duplicate="update", halt_on_error=False
    )
    assert await col.count() == 1
    assert result["created"] == 0
    assert result["errors"] == 0
    ...
    await col.insert(old_doc)
    result = await col.import_bulk(
        [new_doc], on_duplicate="replace", halt_on_error=False
    )
    assert await col.count() == 1
    assert result["created"] == 0
    assert result["errors"] == 0
...
async def test_document_management_via_db(
    db: StandardDatabase, col: StandardCollection
):
    ...
    assert result["_id"] == doc1_id
    assert (await col.get(doc1))["val"] == 100
    assert await col.count() == 3
    ...
    assert result["_id"] == doc1_id
    assert "val" not in (await col.get(doc1))
    assert (await col.get(doc1))["num"] == 300
    assert await col.count() == 3
    ...
    assert result["_id"] == doc1_id
    assert not (await col.has(doc1_id, check_rev=False))
    assert await col.count() == 2
