...
from python_aioarango.collection import Collection
from python_aioarango.database import AsyncDatabase, StandardDatabase
...
from python_aioarango.job import AsyncJob
from tests.helpers import extract

pytestmark = pytest.mark.asyncio


async def wait_on_job(job: AsyncJob):
...
async def wait_on_jobs(db: StandardDatabase):
    """Block until all async jobs are finished."""
    while len(await db.async_jobs("pending")) > 0:
        await asyncio.sleep(0.05)  # pragma: no cover


async def test_async_wrapper_attributes(
    db: StandardDatabase, col: Collection, username
):
...
async def test_async_execute_without_result(
    db: StandardDatabase, col: Collection, docs
):
    ...
    assert extract("_key", elements) == ["1", "2", "3"]


async def test_async_execute_error_in_result(
    db: StandardDatabase, col: Collection, docs
):
...
async def test_async_get_job_status(db: StandardDatabase, bad_db: StandardDatabase):
    async_db = db.begin_async_execution(return_result=True)
...
async def test_async_get_job_result(db: StandardDatabase, bad_db: StandardDatabase):
    async_db = db.begin_async_execution(return_result=True)
...
async def test_async_cancel_job(db: StandardDatabase, bad_db: StandardDatabase):
    async_db = db.begin_async_execution(return_result=True)
...
async def test_async_clear_job(db: StandardDatabase, bad_db: StandardDatabase):
    async_db = db.begin_async_execution(return_result=True)
...
async def test_async_execute_errors(bad_db: StandardDatabase):
...
async def test_async_clear_jobs(
    db: StandardDatabase, bad_db: StandardDatabase, col: Collection, docs
):
    async_db = db.begin_async_execution(return_result=True)
    async_col = async_db.collection(col.name)
...
async def test_async_list_jobs(db: StandardDatabase, col: Collection, docs):
    async_db = db.begin_async_execution(return_result=True)
    async_col = async_db.collection(col.name)
