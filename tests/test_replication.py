...
from python_aioarango.errno import (
    CURSOR_NOT_FOUND,
    DATABASE_NOT_FOUND,
    FORBIDDEN,
    HTTP_NOT_FOUND,
...
async def test_replication_dump_methods(
    db: StandardDatabase,
    bad_db: StandardDatabase,
...
async def test_replication_inventory(
...
async def test_replication_logger_state(
    sys_db: StandardDatabase, bad_db: StandardDatabase, cluster
):
    if cluster:
        pytest.skip("Not tested in a cluster setup")
...
async def test_replication_first_tick(
    sys_db: StandardDatabase, bad_db: StandardDatabase, cluster
):
    if cluster:
        pytest.skip("Not tested in a cluster setup")
...
async def test_replication_applier(
    sys_db: StandardDatabase, bad_db: StandardDatabase, url, cluster
):
    if cluster:
        pytest.skip("Not tested in a cluster setup")
...
async def test_replication_make_slave(
    sys_db: StandardDatabase, bad_db: StandardDatabase, url, replication
):
    if not replication:
...
async def test_replication_cluster_inventory(
    sys_db: StandardDatabase, bad_db: StandardDatabase, cluster
):
    if cluster:
...
async def test_replication_server_id(
    sys_db: StandardDatabase, bad_db: StandardDatabase
):
...
async def test_replication_synchronize(
    sys_db: StandardDatabase, bad_db: StandardDatabase, url, replication
):
    if not replication:
        pytest.skip("Only tested for replication")
