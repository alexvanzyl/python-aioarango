...
from python_aioarango.database import Database, StandardDatabase
...
def test_aql_attributes(db: Database, username):
...
async def test_aql_query_management(
    ...
    result = await db.aql.validate(f"FOR d IN {col.name} RETURN d")
    ...
    elements = [el async for el in cursor]
...
async def test_aql_function_management(db: StandardDatabase, bad_db: StandardDatabase):
...
async def test_aql_cache_management(db: StandardDatabase, bad_db: StandardDatabase):
