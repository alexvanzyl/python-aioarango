...
async def test_permission_management(
    client: ArangoClient, sys_db: StandardDatabase, bad_db: StandardDatabase, cluster
):
    if cluster:
    ...
    assert col_name_1 not in extract("name", await db.collections())
    assert col_name_2 not in extract("name", await db.collections())
    ...
    assert col_name_1 in extract("name", await db.collections())
    assert col_name_2 in extract("name", await db.collections())
