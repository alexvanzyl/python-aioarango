...
from python_aioarango.aql import AQL
from python_aioarango.backup import Backup
from python_aioarango.cluster import Cluster
...
from python_aioarango.executor import (
    AsyncApiExecutor,
    BatchApiExecutor,
    DefaultApiExecutor,
    TransactionApiExecutor,
)
from python_aioarango.formatter import (
    format_body,
    format_database,
...
from python_aioarango.foxx import Foxx
from python_aioarango.graph import Graph
from python_aioarango.job import BatchJob
from python_aioarango.pregel import Pregel
from python_aioarango.replication import Replication
...
from python_aioarango.typings import Json, Jsons, Params
from python_aioarango.utils import get_col_name
from python_aioarango.wal import WAL
...
class Database(ApiGroup):
    ...
    async def delete_task(
        self, task_id: str, ignore_missing: bool = False
    ) -> Result[bool]:
    ...
    async def delete_user(
        self, username: str, ignore_missing: bool = False
    ) -> Result[bool]:
        """Delete a user.
    ...
    async def replace_arangosearch_view(
        self, name: str, properties: Json
    ) -> Result[Json]:
...
class TransactionDatabase(Database):
    ...
    async def begin_transaction(
        ...
        :type max_size: int | None
        """
        return await self._executor.begin(
            read=read,
            write=write,
            exclusive=exclusive,
            sync=sync,
